...
const FLAT_MIXED_ITEMS: IMarkdownNavigatorItem[] = [
  ...URL_ITEM,
  ...RAW_MARKDOWN_ITEM,
];
...
@Component({
  template: ` <div>Footer A Content</div> `,
})
export class FooterAComponent {}

@Component({
  template: ` <div>Global Footer Content</div> `,
})
...
const CHILDREN_URL = 'https://samplechildrenurl.com';
...
describe('MarkdownNavigatorComponent', () => {
  ...
  it('should render empty state when an empty array is passed into items', waitForAsync(
    inject([], async () => {
  ...
  it('should fetch childrenUrl and render', waitForAsync(
    inject([], async () => {
      const itemATitle = 'fetched item A';
      const itemBTitle = 'fetched item B';

      const testData: IMarkdownNavigatorItem[] = [
        { title: itemATitle },
        { title: itemBTitle },
      ];
      const fixture: ComponentFixture<TdMarkdownNavigatorTestComponent> =
  ...
  it('should show proper error messages', waitForAsync(
    inject([], async () => {
      const invalidMarkdownUrl = 'https://invalid_markdown_url.com';
      const invalidChildrenUrl = 'https://invalid_children_url.com';
      const ITEMS_WITH_ERRORS: IMarkdownNavigatorItem[] = [
        ...
        {
          title: 'Invalid markdown and children url',
          url: invalidMarkdownUrl,
          childrenUrl: invalidChildrenUrl,
        },
      ];
      const invalidMarkdownUrlOptions: object = {
        status: 404,
      ...
      const invalidMarkdownUrlRequest: TestRequest =
        httpTestingController.expectOne(invalidMarkdownUrl);
      ...
      getItem(fixture, 0).click();
      const invalidChildrenUrlRequest: TestRequest =
        httpTestingController.expectOne(invalidChildrenUrl);
      ...
      const invalidMarkdownUrlRequest2: TestRequest =
        httpTestingController.expectOne(invalidMarkdownUrl);
      ...
      const invalidMarkdownUrlRequest3: TestRequest =
        httpTestingController.expectOne(invalidMarkdownUrl);
      invalidMarkdownUrlRequest3.flush(message, invalidMarkdownUrlOptions);
      const invalidChildrenUrlRequest2: TestRequest =
        httpTestingController.expectOne(invalidChildrenUrl);
  ...
  it('should render list and markdown side by side', waitForAsync(
    inject([], async () => {
      ...
      if (!ITEMS_AT_SAME_LEVEL_AS_MARKDOWN[0].children) {
        return;
      }
  ...
  it('should be able to start at a certain item by passing a reference to that item', waitForAsync(
    inject([], async () => {
      ...
      fixture.componentInstance.startAt = DEEPLY_NESTED_TREE[0].children
        ? DEEPLY_NESTED_TREE[0].children[0]
        : [];
      await wait(fixture);
      expect(getItem(fixture, 0).textContent).toContain('A');
  ...
  it('should not jump anywhere if path is invalid', waitForAsync(
    inject([], async () => {
      ...
      fixture.componentInstance.items = DEEPLY_NESTED_TREE;
      fixture.componentInstance.startAt = [
        { id: 'A_ID' },
        { id: 'NON_EXISTING_ID' },
      ];
      await wait(fixture);
  ...
  it('should be able to jump to start at a certain item by referencing a path of items that depends on children_url', waitForAsync(
    inject([], async () => {
      ...
      const childrenUrl2 = 'https://anothersamplechildrenurl.com';
      const childrenUrlRequest1: IMarkdownNavigatorItem[] = [
        { id: '2a', title: '2a', childrenUrl: childrenUrl2 },
      ];
      const childrenUrlRequest2: IMarkdownNavigatorItem[] = [
        { title: '3c', id: '3c' },
      ];
  ...
  it('should match URLs by filename ignoring query params and hash fragments', waitForAsync(
    inject([], async () => {
      ...
      // Test same filename different paths
      expect(
  ...
  it('should preserve history stack when navigating through internal links (shouldReset=false)', waitForAsync(
    inject([], async () => {
      ...
      // Navigate to child via internal link (shouldReset = false)
      await component['_jumpTo']({ id: 'child' }, undefined, false);
      await wait(fixture);

      // Should preserve parent and add child
  ...
  it('should handle clicks on nested elements inside links using closest()', waitForAsync(
    inject([], async () => {
      ...
      // Create a mock link with nested element
      ...
      // Verify closest() finds the anchor
  ...
  it('should preserve breadcrumbs when mixing menu navigation and internal links', waitForAsync(
    inject([], async () => {
      ...
      // Internal link to settings (shouldReset = false)
      ...
      // Internal link to advanced (shouldReset = false)
      await component['_jumpTo']({ id: 'advanced' }, undefined, false);
      await wait(fixture);

      // Should have all three in breadcrumbs
  ...
  it('should not duplicate parent items in breadcrumbs when clicking internal links to nested items', waitForAsync(
    inject([], async () => {
      ...
      // Click internal link to API Reference
      await component['_jumpTo']({ id: 'api' }, undefined, false);
      await wait(fixture);
      ...
      expect(component.historyStack.length).toBe(2);
      expect(component.historyStack[0].id).toBe('docs');
      expect(component.historyStack[1].id).toBe('api');
    }),
  ));
});
