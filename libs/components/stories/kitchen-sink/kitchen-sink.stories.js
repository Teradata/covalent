...
import * as kitchenSinkContent from '../demos/material-web-comparison.content.html';
...
export const Basic = ({}) => {
  document.addEventListener(
    ...
    () => {
      ...
      // Chips
      const chipSetEls = Array.from(
        mainEl.querySelectorAll('.mdc-chip-set, .mdc-evolution-chip-set')
      );
      chipSetEls.forEach((el) => new MDCChipSet(el));

      // Text field
      const textFieldEls = Array.from(
        mainEl.querySelectorAll('.mdc-text-field')
      );
      ...
      // Top app bar
      const topAppBarEls = Array.from(
        mainEl.querySelectorAll('.mdc-top-app-bar')
      );
      ...
      const themeBuilderDrawerEl = document.querySelector(
