import { routeBuilder, IScopedRouteBuilder } from 'app/utilities/route-builder';

export const echartsRoutesCategories: any = [
  { name: '', nested: false },
  { name: 'Base', nested: false },
  { name: 'Chart Types', nested: false, color: 'blue-800' },
  { name: 'Atomic Components', nested: false, color: 'orange-A400' },
];

const [root, base, chartTypes, atomic] = echartsRoutesCategories;

export const echartsContentDetails: any = [
  {
    name: 'Chart',
    id: 'chart',
    description: 'Base component that instantiates an echarts object',
    apiDocUrl: 'platform/echarts/base/README.md',
    showExampleTab: true,
    showOverviewDemo: false,
    hideExamples: true,
    hideOverview: true,
    overviewDocUrl: '',
    icon: 'multiline_chart',
    category: base.name,
    route: '/visualizations/echarts/chart',
    color: '',
  },
  {
    name: 'Bar',
    id: 'bar',
    description: '',
    apiDocUrl: 'platform/echarts/bar/README.md',
    showExampleTab: true,
    showOverviewDemo: false,
    showOverview: true,
    hideOverview: false,
    category: chartTypes.name,
    overviewDocUrl: '',
    icon: 'bar_chart',
    route: '/visualizations/echarts/bar',
    demo: () => import('./echarts-demos/bar/demos/bar-demo.module').then((mod: any) => mod.BarDemoModule),
  },
  // {
  //   name: 'Combination',
  //   id: 'combination',
  //   description: '',
  //   apiDocUrl: 'platform/echarts/combination/README.md',
  //   showExampleTab: true,
  //   showOverviewDemo: true,
  //   overviewDocUrl: '',
  //   icon: 'multiline_chart',
  //   hideOverview: false,
  //   category: chartTypes.name,
  //   route: '/echarts/combination',
  //   color: '',
  //   demo: () =>
  //     import('./echarts-demos/combination/demos/combination-demo.module').then((mod: any) => mod.CombinationDemoModule),
  // },
  {
    name: 'Graph',
    id: 'graph',
    description: '',
    apiDocUrl: 'platform/echarts/graph/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'grain',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/graph',
    color: '',
    demo: () => import('./echarts-demos/graph/demos/graph-demo.module').then((mod: any) => mod.GraphDemoModule),
  },
  {
    name: 'Map',
    id: 'map',
    description: '',
    apiDocUrl: 'platform/echarts/map/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'map',
    hideOverview: false,
    category: chartTypes.name,
    route: '/echarts/map',
    color: '',
    demo: () => import('./echarts-demos/map/demos/map-demo.module').then((mod: any) => mod.MapDemoModule),
  },
  {
    name: 'Line',
    id: 'line',
    description: '',
    apiDocUrl: 'platform/echarts/line/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'show_chart',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/line',
    color: '',
    demo: () => import('./echarts-demos/line/demos/line-demo.module').then((mod: any) => mod.LineDemoModule),
  },
  {
    name: 'Pie',
    id: 'pie',
    description: '',
    apiDocUrl: 'platform/echarts/pie/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    hideOverview: false,
    category: chartTypes.name,
    overviewDocUrl: '',
    icon: 'insert_chart',
    route: '/visualizations/echarts/pie',
    demo: () => import('./echarts-demos/pie/demos/pie-demo.module').then((mod: any) => mod.PieDemoModule),
  },
  {
    name: 'Sankey',
    id: 'sankey',
    description: '',
    apiDocUrl: 'platform/echarts/sankey/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'call_split',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/sankey',
    color: '',
    demo: () => import('./echarts-demos/sankey/demos/sankey-demo.module').then((mod: any) => mod.SankeyDemoModule),
  },
  {
    name: 'Scatter',
    id: 'scatter',
    description: '',
    apiDocUrl: 'platform/echarts/scatter/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'bubble_chart',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/scatter',
    color: '',
    demo: () => import('./echarts-demos/scatter/demos/scatter-demo.module').then((mod: any) => mod.ScatterDemoModule),
  },
  {
    name: 'Tree',
    id: 'tree',
    description: '',
    apiDocUrl: 'platform/echarts/tree/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'share',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/tree',
    color: '',
    demo: () => import('./echarts-demos/tree/demos/tree-demo.module').then((mod: any) => mod.TreeDemoModule),
  },
  {
    name: 'Treemap',
    id: 'treemap',
    description: '',
    apiDocUrl: 'platform/echarts/treemap/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'dashboard',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/treemap',
    color: '',
    demo: () => import('./echarts-demos/treemap/demos/treemap-demo.module').then((mod: any) => mod.TreemapDemoModule),
  },
  {
    name: 'Wordcloud',
    id: 'wordcloud',
    description: '',
    apiDocUrl: 'platform/echarts/wordcloud/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'cloud_queue',
    hideOverview: false,
    category: chartTypes.name,
    route: '/visualizations/echarts/wordcloud',
    color: '',
    demo: () =>
      import('./echarts-demos/wordcloud/demos/wordcloud-demo.module').then((mod: any) => mod.WordcloudDemoModule),
  },
  {
    name: 'Axis',
    id: 'axis',
    description: 'Control chart axis declaratively',
    apiDocUrl: 'platform/echarts/base/axis/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    hideExamples: true,
    overviewDocUrl: '',
    icon: 'trending_flat',
    hideOverview: true,
    category: atomic.name,
    route: '/visualizations/echarts/axis',
    color: '',
  },
  {
    name: 'Dataset',
    id: 'dataset',
    description: 'Add flexibility to manage chart data independently',
    apiDocUrl: 'platform/echarts/base/dataset/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    hideExamples: true,
    overviewDocUrl: '',
    icon: 'view_list',
    hideOverview: true,
    category: atomic.name,
    route: '/visualizations/echarts/dataset',
    color: '',
  },
  {
    name: 'Tooltip',
    id: 'tooltip',
    description: 'Display tooltips on charts',
    apiDocUrl: 'platform/echarts/tooltip/README.md',
    showExampleTab: true,
    showOverviewDemo: true,
    hideExamples: true,
    overviewDocUrl: '',
    icon: 'info',
    hideOverview: true,
    category: atomic.name,
    route: '/visualizations/echarts/tooltip',
    color: '',
  },
  {
    name: 'Toolbox',
    id: 'toolbox',
    description: 'Add a multi functional toolbox to charts',
    apiDocUrl: 'platform/echarts/toolbox/README.md',
    showExampleTab: true,
    hideExamples: true,
    showOverviewDemo: true,
    overviewDocUrl: '',
    icon: 'art_track',
    hideOverview: true,
    category: atomic.name,
    route: '/visualizations/echarts/toolbox',
    color: '',
  },
];

export const setEchartRoutes: IScopedRouteBuilder = routeBuilder(echartsContentDetails);
