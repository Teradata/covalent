...
describe('Service: TdDataTableService', () => {
  ...
  it('should sort data a by nested value', () => {
    let d: any[] = service.sortData(nestedDataExample, 'score.nested.value', TdDataTableSortingOrder.Descending);
    expect(d[0].score.nested.value).toBe(89);
  });
  it('should sort data a non-nested value', () => {
    let d: any[] = service.sortData(nestedDataExample, 'value', TdDataTableSortingOrder.Ascending);
    expect(d[0].value).toBe(34);
  });
});

let nestedDataExample: any[] = [
  {
    'value': 78,
    'score': {
      'value': 8,
