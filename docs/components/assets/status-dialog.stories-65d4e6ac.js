...
`;var F=Object.defineProperty,I=Object.getOwnPropertyDescriptor,p=(t,e,o,i)=>{for(var a=i>1?void 0:i?I(e,o):e,c=t.length-1,v;c>=0;c--)(v=t[c])&&(a=(i?v(e,o,a):v(a))||a);return i&&a&&F(e,o,a),a};let n=class extends P{constructor(){super(...arguments),this.state="error",this.expandDetails=!1,this.showDetailsLink=!1}firstUpdated(){if(super.firstUpdated(),this._details){const t=this._details.assignedNodes({flatten:!0});this.showDetailsLink=t.some(e=>e.nodeType===Node.ELEMENT_NODE)}}toggleDetails(){this.expandDetails=!this.expandDetails}_getStatusIcon(){switch(this.state){case"positive":return"check";default:return this.state}}renderDetails(){const t={"status-dialog__details":!0,visible:this.expandDetails},e={open:!this.expandDetails,close:this.expandDetails},o=s`<cv-icon
    ...
    >`;return s`${this.showDetailsLink?s`<div>
      ...
      <cv-icon-button
        ...
        @click=${this.close}
      ></cv-icon-button>
    </span>`}render(){const t={[H.STACKED]:this.stacked,error:this.state==="error",positive:this.state==="positive",warning:this.state==="warning"},e=this.renderHeading(),o=this.renderDetails(),i={"mdc-dialog__actions":!this.hideActions},a={"mdc-dialog__content":!0,"padding-bottom":this.hideActions};return s` <div
    ...
    >
      ...
      ${T(j)}
    `];p([U("#details")],n.prototype,"_details",2);p([W({type:String})],n.prototype,"state",2);p([q()],n.prototype,"expandDetails",2);p([q()],n.prototype,"showDetailsLink",2);n=p([B("cv-status-dialog")],n);const N={error:`<p style="margin: 0;">
...
`,_t={title:"Components/Status Dialog",argTypes:{state:{options:["error","positive","warning"],control:{type:"select"}}},args:{state:"error",open:!1,hideActions:!1,heading:"Basic heading for the dialog"}},h=({state:t,hideActions:e,heading:o})=>(document.addEventListener("DOMContentLoaded",()=>{document.body.querySelector("#dialog-button").addEventListener("click",()=>{const c=document.body.querySelector("#dialog");c.open=!0})},{once:!0}),`
    ...
    </cv-status-dialog>`),K=({state:t,hideActions:e,heading:o})=>(document.addEventListener("DOMContentLoaded",()=>{document.body.querySelector("#dialog-button-2").addEventListener("click",()=>{const a=document.body.querySelector("#dialog-2");a.open=!0})},{once:!0}),`
...
}`,...(O=(E=u.parameters)==null?void 0:E.docs)==null?void 0:O.source}}};const yt=["Basic","Positive","Warning","Error","WithDetails"];export{m as Basic,g as Error,r as Positive,l as Warning,u as WithDetails,yt as __namedExportsOrder,_t as default};
